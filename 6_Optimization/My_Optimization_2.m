...
% scales = [1.0 1.5]; % quantization scale factor, the larger, the lower PSNR
...
for scaleIdx=1:numel(scales)
    ...
    for i=2:21
        ...
        ref_frame_ycbcr_group=rec_frame_ycbcr(earliest_im:i-1);  % group of reference images
        num_ref_frames=numel(ref_frame_ycbcr_group);
        current_frame_rgb=sequence{i};
        current_frame_ycbcr=ictRGB2YCbCr(current_frame_rgb);
        
        %         for j=1:num_ref_frames
        %             ref_frame_ycbcr_temp=ref_frame_ycbcr{i};
        %             motion_vector=SSD_HalfPel(ref_frame_ycbcr_temp(:,:,1),current_frame_ycbcr(:,:,1));
        %             motion_compensation_image_ycbcr=SSD_rec_HalfPel(ref_frame_ycbcr,motion_vector);
        %         end
        
        [motion_vector,ref_frame_indices] = SSD_HalfPel_MultipleRefFrames(ref_frame_ycbcr_group, current_frame_ycbcr);
        motion_compensation_image_ycbcr=SSD_rec_HalfPel_MultiRefFrames(ref_frame_ycbcr_group,motion_vector,ref_frame_indices);
        ...
        %%%%%%%%%%%%%%%%%% update adaptive Huffman code %%%%%%%%%%%%%%%%%%%%
        % seems to to be much time-consuming, but improves only little
        % bit-rate
        ...
        %         sum_pmf_ind_mv=sum_pmf_ind_mv+pmf_ind_mv;
        %         sum_pmf_ind_mv=sum_pmf_ind_mv/sum(sum_pmf_ind_mv);
        %         [BinaryTree_mv,HuffCode_mv,BinCode_mv,Codelengths_mv]=buildHuffman(sum_pmf_ind_mv);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if i==2
            ...
            % Huffman code for ref_frame_indices
            pmf_ind_ref=hist(ref_frame_indices(:),1:20);
            pmf_ind_ref=pmf_ind_ref/sum(pmf_ind_ref);
            [BinaryTree_ref,HuffCode_ref,BinCode_ref,Codelengths_ref]=buildHuffman(pmf_ind_ref);
        end
        ...
        bytestream3=enc_huffman_new(ref_frame_indices(:)-min(ref_frame_indices(:))+1,BinCode_ref,Codelengths_ref); % rate for motion vector
        bitPerPixel1 = (numel(bytestream1)*8) / (numel(sequence{i})/3);
        bitPerPixel2 = (numel(bytestream2)*8) / (numel(sequence{i})/3);
        bitPerPixel3 = (numel(bytestream3)*8) / (numel(sequence{i})/3);
        rate_mat_video(i,scaleIdx)=bitPerPixel1+bitPerPixel2+bitPerPixel3;
...
% plot
figure;
plot(rate_video,psnr_video,'-*','LineWidth',1);hold on;
...
x=[3.54 1.86 1.08 0.61 0.52 0.38 0.31 0.24 0.22 0.21];
y=[45.59 40.64 38.04 35.8 35.11 33.89 32.99 31.63 30.62 30.27];
plot(x,y,'-*','LineWidth',1); % My_Optimization_1:half-pel accuracy
...
legend('My Optimization2','My Optimization1','Baseline2 (chapter5)','Baseline1 (chapter4)','FontSize',12);